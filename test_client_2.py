...
for chunk in response.iter_content(chunk_size=None):
    ...
    if len(buffer) >= 10:  # Adjust the threshold as needed
        print(buffer.decode('utf-8'), flush=True)  # Process the buffer as needed
        buffer = b""  # Clear the buffer

# Process any remaining data in the buffer
if buffer:
    print(buffer.decode('utf-8'), flush=True)  # Process the remaining data in the buffer
